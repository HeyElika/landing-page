...
`stepsSplit` lists every step as copy beside a single visual. It is deliberately static — no sticky stage, no scroll-driven sequence. A reader deciding whether to start wants to see how many steps there are and compare them; revealing one at a time hides the length of the process behind a gesture.

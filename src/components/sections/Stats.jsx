  ...
  return (
    <section className={['l-band', bandTone].filter(Boolean).join(' ')}>
      <div className="l-container l-stack l-stack--600">
        {title && <h2 className={['heading-lg-bold', 't-center', onDark ? 't-on-dark' : ''].filter(Boolean).join(' ')}>{title}</h2>}
        <ul className={`l-grid l-grid--${Math.min(items.length, 4) || 3}`}>
          {items.map((item) => (
            <li key={item.label} className="l-stack l-stack--100 t-center">
              <span className={['heading-lg-bold', onDark ? 't-on-dark' : ''].filter(Boolean).join(' ')}>{item.value}</span>
              <span className={['body-sm-regular', onDark ? 't-on-dark-subtle' : 't-subtle'].join(' ')}>{item.label}</span>
            </li>
          ))}
        </ul>
      </div>
